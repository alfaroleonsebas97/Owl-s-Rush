...
const numberOfCardsPerPlayer = parseInt(window.localStorage.getItem('cardsPerPlayer'), 10);
const numMaxOwls = 5;
...
export default class Player {
  constructor(id, socket) {
    ...
    this.owlsInNest = 0;
    this.winner = false;
    this.tableState = tableCellState;
  ...
  sendFirstSetOfCards() {
    let cards = '';
    for (let i = 0; i < this.cardsInHand.length; i += 1) {
      cards += `{"card": "${this.cardsInHand[i].cardName}"}`;
      if (i != this.cardsInHand.length - 1) {
        cards += ', ';
      }
    }
    this.socket.send(`{ "type": "newSetCards", "sessionCode": "${sessionCode}", "playerId": "${myPlayerId}", "setCards": [${cards}]}`);
  }
  ...
  getCard() {
    ...
    document.getElementById('deck_img').addEventListener('click', () => {
      if (this.myTurn && this.deckWaiting) {
        for (let j = 0; j < numberOfCardsPerPlayer; j += 1) {
  ...
  endTurn() {
    this.myTurn = false;
    this.socket.send(`{ "type": "turnFinished", "sessionCode": "${sessionCode}", "playerId": "${myPlayerId}", "owlMoved": "${this.selectedOwlNum}", "cardUsed": "${this.selectedCardNumber}", "newCard": "${this.cardsInHand[this.selectedCardNumber].cardName}", "score": "${this.myScore}", "winner": "${this.winner}", "turnFinished": "${!this.doubleTurn}" }`);
  ...
  applyRainbowSpecialCard() {
    ...
    const changeRainbowColor = () => {
      if (rainbowElement.value !== '0') {
        ...
        this.enableCards();
        this.socket.send(`{ "type": "specialCardToColor", "sessionCode": "${sessionCode}", "playerId": "${myPlayerId}", "cardUsed": "${this.selectedCardNumber}", "newCardName": "${this.cardsInHand[this.selectedCardNumber].cardName}"}`);
